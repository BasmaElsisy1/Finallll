...
//Pixabay data
const pixabay_API = process.env.API;
...
function  generateData (e){
    ...
    .then (function(data){
        console.log(data);
        postData ('/add', {lat: data.main.lat, lon: data.main.lon, country:data.main.country})
    }) .then(userView());

const generateWeth = async  (weatherbit_baseurl, weather_API, lat,lng)=>{
    const res = await fetch (`${weatherbit_baseurl}lat=${lat}&lon=${lng}&key=${weather_API}`);
    try {
        const data = await res.json();
        console.log (data);
        return (data);
...
.then (function(data){
    console.log(data);
    postData ('/add', {temp: data.main.temp , description: data.main.description})
    .then (userView());
})
...
.then(function(data){
    console.log(data);
    postData ('/add', {picture: data.main.picData})
    .then (userView());
})
